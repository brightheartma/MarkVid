...
│
├── src/
│   ├── extract_media.py     # 提取音频和关键帧（增量）
│   ├── transcribe_audio.py  # 双擎转录：Groq API 优先，ASPH 限流时本地 faster-whisper 接管
...
| **双擎转录** | Groq API 优先；ASPH 限流时自动切换本地 `faster-whisper`，无需手动重试 |
...
```bash
# 完整流水线
bash scripts/run_pipeline.sh                   # 全流程（增量）
bash scripts/run_pipeline.sh --skip-note      # 提取+转录，跳过笔记（新视频用）
bash scripts/run_pipeline.sh --transcribe-only # 仅转录（已提取过）
bash scripts/run_pipeline.sh --note-only      # 仅生成笔记和索引（已转录过）
...
bash scripts/run_pipeline.sh --force-transcribe  # 强制重新转录所有
bash scripts/run_pipeline.sh --force-all         # 全部强制重处理
TRANSCRIBE_FORCE=true .venv/bin/python src/transcribe_audio.py  # 同上（转录单步）
```
...
| `LOCAL_WHISPER_MODEL` | `large-v3-turbo` | 限流时本地 fallback 模型（`medium` 更省内存） |

---

## 技术栈

- **转录**：[Groq API](https://console.groq.com) `whisper-large-v3`（云端优先）→ `faster-whisper`（限流时本地托底）
